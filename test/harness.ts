...
export async function assertThrows(
  ...
  try {
    ...
    if (!pred(error)) {
      throw new AssertionError(`threw, but predicate rejected the error: ${(error as Error)?.name ?? String(error)}`);
    }
    return;
  }
...
export function isInstance<E extends new (...args: never[]) => Error>(ctor: E): (error: unknown) => boolean {
